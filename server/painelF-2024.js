...
//const connection2 = adodb.open('Provider=Microsoft.ACE.OLEDB.16.0;Data Source=C:\\CNDServidor\\CND14\\Concoordbd.accdb;', true);
...
function calcularPainel(periodo, segmento, fvendas) {
  return new Promise((resolve, reject) => {
    ...
    copiarBD() //CRIA UMA CÓPIA DO BANCO DE DADOS >>>>
      .then((copyPath) => {
        ...
        buscaResultadosCanal(periodoSQL, segmentoSQL, fvendasSQL)
          .then((resultado) => {
            ...
            buscaResultadosMensais(periodoSQL, segmentoSQL, fvendasSQL)
              .then((resultado) => {
...
async function buscaResultadosCanal(periodoCanal, segmentoCanal, fvendasCanal) {
  try {

    const sql = `SELECT CANAL, SEGMENTO, ${periodoCanal} 
    FROM PE_REDE
    INNER JOIN PE_2024 ON PE_REDE.MCU = PE_2024.MCU
    WHERE SEGMENTO = ${segmentoCanal} AND F_VENDAS = ${fvendasCanal}
...
async function buscaResultadosMensais(periodoCanal, segmentoCanal, fvendasCanal) {
  try {
